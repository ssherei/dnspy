...
class initialize():
		...
		self.query_types = ['A','NS','MX','TXT','SOA']
	...
	def packet_magic(self,b=None):
		...
		for self.query in self.cur:
			...
			# set packet destination to name server to queryA
				for self.query_type in self.query_types:
					self.req = DNS.DnsRequest(name = self.query[0], qtype = self.query_type, server = self.ns[0])
					print "[*] Query: %s\r\n" % self.req.args
					#time.sleep(1)

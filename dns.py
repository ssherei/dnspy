...
class initialize():
	...
	# we check to see if there is a record for same server exist cursor.fetchone() returns None if no data is retrieved "data with same domain, name server, query Type"
				self.cur.execute("select * from %s where qname=(select id from watched where domain=%%s) and qtype=%%s and dst_ns=%%s and an_rdata = %%s" % self.table, (self.a['name'],self.a['typename'],self.ans.args['server'], self.a['data']))
				if self.cur.fetchone() == None:

	# if no data is retrieved by  cursor.fetchone()
	# insert new data in corresponding table
					self.cur.execute("insert into %s(qname,qtype,dst_ns,an_rdata,an_type) values((select id from watched where domain=%%s),%%s,%%s,%%s,%%s)" % self.table, (self.a['name'], self.a['typename'], self.ans.args['server'], self.a['data'], self.a['typename']))

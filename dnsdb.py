...
try:
	...
	with con:
		...
		cur.execute("create table baseline(id int primary key auto_increment,  qname int, A varchar(100), NS varchar(300), MX varchar(300), TXT varchar(300), SOA varchar(300), dst_ns int,time_stamp timestamp, constraint fk_qname foreign key (qname) references watched(id) on update cascade on delete cascade, constraint fk_dst_ns foreign key (dst_ns) references name_servers(id) on update cascade on delete cascade)")

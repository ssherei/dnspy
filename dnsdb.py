...
try:
	...
	with con:
		...
		cur.execute("create table baseline(id int primary key auto_increment,  qname int, A varchar(100), NS varchar(300), MX varchar(300), TXT varchar(300), SOA varchar(300), dst_ns varchar(50),time_stamp timestamp, constraint fk_qname foreign key (qname) references watched(id) on update cascade on delete cascade)")

		print "[*] Creating Table latest_update"
		cur.execute("drop table if exists latest_update")
		cur.execute("create table latest_update(id int primary key auto_increment,  qname int, A varchar(100), NS varchar(300), MX varchar(300), TXT varchar(300), SOA varchar(300), dst_ns varchar(50),time_stamp timestamp, constraint fk_qname_latest foreign key (qname) references watched(id) on update cascade on delete cascade)")
